...
#ifndef __LICENCE_COMMAND_H__
#define __LICENCE_COMMAND_H__

#include "CommandInterface.h"

class LicenceCommand : public CommandInterface
{
public:
	...
	// execute the command - display software licensing terms
	int execute();

	// return the command name as a printable string
	const char* getCommandName() const { return "LicenceCommand"; }

protected:

	// output LGPL licence text
	void printLicence();

private:

	// copy constructor and operators private and not implemented
	LicenceCommand(const LicenceCommand& other);
	LicenceCommand& operator=(const LicenceCommand& other);
};

#endif // __LICENCE_COMMAND_H__
