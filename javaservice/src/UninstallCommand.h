...
#ifndef __UNINSTALL_COMMAND_H__
#define __UNINSTALL_COMMAND_H__

#include "ServiceCommand.h"

class UninstallCommand : public ServiceCommand
{
public:

	UninstallCommand(const char* _serviceName) : ServiceCommand(_serviceName) {}

	~UninstallCommand() {}

	int execute();

	const char* getCommandName() const { return "UninstallCommand"; }

protected:

	bool uninstallService();

	bool deleteServiceConfig();

private:

	UninstallCommand(const UninstallCommand& other);
	UninstallCommand& operator=(const UninstallCommand& other);
};

#endif // #ifndef __UNINSTALL_COMMAND_H__
